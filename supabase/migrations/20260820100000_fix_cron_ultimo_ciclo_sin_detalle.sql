-- Migración: Fix cron facturación - calcular último ciclo_fin también desde cuentas_cobrar directamente
-- Problema: el cron calculaba el "último ciclo" sólo a través del JOIN con cxc_detalle,
-- lo que causaba que si una nota manual existía en cuentas_cobrar pero su cxc_detalle
-- aún no había sido insertado (o fue insertado con retraso), el cron no la detectaba
-- y generaba un borrador automático para un período ya cubierto.
-- Solución: ampliar la búsqueda del último ciclo_fin para incluir cuentas_cobrar.ciclo_fin
-- directamente (sin exigir join con detalle), tomando el mayor entre ambas fuentes.
...
BEGIN
    ...
    FOR v_config IN (
        ...
        FOR v_alumno IN (
            ...
            --   2. cuentas_cobrar.ciclo_fin directamente (cubre notas sin detalle o con detalle tardío)
            -- Se toma el MAYOR de ambos para garantizar que no se regenere un período ya cubierto.
            SELECT GREATEST(
                -- Fuente 1: notas con cxc_detalle de tipo mensualidad
                (
                    SELECT cc.ciclo_fin
                ...
                -- Fuente 2: notas de mensualidad en cuentas_cobrar con ciclo_fin directo
                -- (por descripción, para capturar notas sin detalle o con detalle aún no insertado)
                (
                    SELECT cc.ciclo_fin
                    FROM public.cuentas_cobrar cc
                    WHERE cc.alumno_id = v_alumno.id
            ...
            -- REGLA ESTRICTA DE 7 DÍAS DESDE LA FECHA DE EMISIÓN:
            -- 1. No generar si la fecha de emisión aún no llega (es en el futuro).
            IF v_fecha_emision > p_hoy THEN
                CONTINUE;
            END IF;

            -- 2. No generar si la fecha de emisión fue hace más de 7 días.
