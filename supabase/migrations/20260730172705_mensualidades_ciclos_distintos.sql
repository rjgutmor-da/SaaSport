-- Permite incluir varias mensualidades en una misma Nota de Servicio.
-- Cada linea conserva su propio ciclo y periodo estadistico, sin permitir
-- periodos repetidos para el mismo alumno.

ALTER TABLE public.cxc_detalle
  ADD COLUMN IF NOT EXISTS ciclo_inicio DATE,
  ADD COLUMN IF NOT EXISTS ciclo_fin DATE,
  ADD COLUMN IF NOT EXISTS periodo_estadistico DATE;

ALTER TABLE public.cxc_detalle
  DROP CONSTRAINT IF EXISTS cxc_detalle_ciclo_mensualidad_check;

ALTER TABLE public.cxc_detalle
  ADD CONSTRAINT cxc_detalle_ciclo_mensualidad_check
  CHECK (
    (ciclo_inicio IS NULL AND ciclo_fin IS NULL AND periodo_estadistico IS NULL)
    OR (
      ciclo_inicio IS NOT NULL
      AND ciclo_fin IS NOT NULL
      AND periodo_estadistico IS NOT NULL
      AND ciclo_fin >= ciclo_inicio
      AND periodo_estadistico = date_trunc('month', periodo_estadistico)::DATE
    )
  );

-- Las mensualidades historicas ya tienen el ciclo canonico en la cabecera.
UPDATE public.cxc_detalle d
SET ciclo_inicio = cc.ciclo_inicio,
    ciclo_fin = cc.ciclo_fin,
    periodo_estadistico = cc.periodo_estadistico
FROM public.cuentas_cobrar cc,
     public.catalogo_items ci
WHERE cc.id = d.cuenta_cobrar_id
  AND ci.id = d.catalogo_item_id
  AND ci.escuela_id = d.escuela_id
  AND lower(btrim(ci.nombre)) = 'mensualidad'
  AND cc.ciclo_inicio IS NOT NULL
  AND cc.ciclo_fin IS NOT NULL
  AND cc.periodo_estadistico IS NOT NULL
  AND d.periodo_estadistico IS NULL
  AND (
    SELECT count(*)
    FROM public.cxc_detalle otro
    JOIN public.catalogo_items otro_item
      ON otro_item.id = otro.catalogo_item_id
    WHERE otro.cuenta_cobrar_id = d.cuenta_cobrar_id
      AND lower(btrim(otro_item.nombre)) = 'mensualidad'
  ) = 1;

CREATE INDEX IF NOT EXISTS cxc_detalle_periodo_estadistico_idx
  ON public.cxc_detalle (escuela_id, periodo_estadistico)
...
DROP TRIGGER IF EXISTS validar_ciclo_mensualidad_detalle_trigger
  ON public.cxc_detalle;

CREATE TRIGGER validar_ciclo_mensualidad_detalle_trigger
BEFORE INSERT OR UPDATE OF
  escuela_id, cuenta_cobrar_id, catalogo_item_id,
  ciclo_inicio, ciclo_fin, periodo_estadistico
ON public.cxc_detalle
FOR EACH ROW
EXECUTE FUNCTION public.validar_ciclo_mensualidad_detalle();
...
DROP TRIGGER IF EXISTS validar_reactivacion_periodos_mensualidad_trigger
  ON public.cuentas_cobrar;

CREATE TRIGGER validar_reactivacion_periodos_mensualidad_trigger
BEFORE UPDATE OF anulada, alumno_id, escuela_id
ON public.cuentas_cobrar
FOR EACH ROW
EXECUTE FUNCTION public.validar_reactivacion_periodos_mensualidad();
