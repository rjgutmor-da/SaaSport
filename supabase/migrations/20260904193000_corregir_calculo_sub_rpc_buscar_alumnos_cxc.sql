-- Migration: 20260904193000_corregir_calculo_sub_rpc_buscar_alumnos_cxc.sql
-- Corrige el cálculo de la categoría Sub en rpc_buscar_alumnos_cxc.
-- En el fútbol formativo, la categoría Sub se determina exclusivamente por la diferencia
-- entre el año actual de gestión y el año de nacimiento del alumno (Año Actual - Año Nacimiento).
-- Previamente se utilizaba extract(year from age(CURRENT_DATE, p.fecha_nacimiento)), lo que
-- calculaba la edad cronológica exacta y asignaba una categoría inferior a los alumnos cuyo
-- cumpleaños aún no había transcurrido en el año (ej. nacido en sept 2012 aparecía como Sub-13 en vez de Sub-14).
...
DECLARE
  ...
  v_resultado jsonb;
BEGIN
  SELECT u.escuela_id, u.sucursal_id, u.rol::text, COALESCE(e.zona_horaria, 'America/La_Paz')
    INTO v_escuela_id, v_sucursal_actor, v_rol, v_zona_horaria
  FROM public.usuarios AS u
  LEFT JOIN public.escuelas AS e ON e.id = u.escuela_id
  WHERE u.id = v_uid
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'sub', CASE WHEN p.fecha_nacimiento IS NULL THEN NULL
          ELSE (EXTRACT(year FROM timezone(COALESCE(v_zona_horaria, 'America/La_Paz'), now())::date)::integer - EXTRACT(year FROM p.fecha_nacimiento)::integer) END,
