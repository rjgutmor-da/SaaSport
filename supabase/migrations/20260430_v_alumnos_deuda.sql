-- ==============================================================================
-- SaaSport: Migración de Vista v_alumnos_deuda con Soporte de Búsqueda sin Acentos
-- Fecha: 2026-04-30
-- ==============================================================================
...
CREATE OR REPLACE VIEW public.v_alumnos_deuda AS
 ...
 SELECT a.id AS alumno_id,
    ...
    COALESCE(sum(cc.saldo_pendiente) FILTER (WHERE (cc.anulada = false)), (0)::numeric) AS saldo_pendiente,
    count(cc.id) FILTER (WHERE (((cc.estado)::text = ANY (ARRAY[('pendiente'::character varying)::text, ('parcial'::character varying)::text])) AND (cc.anulada = false))) AS cxc_pendientes,
    ...
    a.fecha_inicio,
    unaccent(lower(a.nombres)) AS nombres_search,
