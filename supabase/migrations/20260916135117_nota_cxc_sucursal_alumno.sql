-- Sucursal por defecto del alumno; conserva la sucursal en ediciones.
-- Administrador/Asistente con sucursal NULL mantienen alcance de escuela como en CxC.
BEGIN;
...
ALTER POLICY inventario_saldos_select ON public.inventario_saldos
  USING (
    escuela_id = (SELECT public.current_user_escuela_id())
    AND (
      (SELECT public.current_user_rol()) = 'SuperAdministrador'
      OR ((SELECT public.current_user_rol()) IN ('Administrador', 'Asistente')
        AND ((SELECT private.current_user_sucursal_id()) IS NULL
          OR sucursal_id = (SELECT private.current_user_sucursal_id())))
    )
  );

ALTER POLICY inventario_aperturas_select ON public.inventario_aperturas
  USING (
    escuela_id = (SELECT public.current_user_escuela_id())
    AND (
      (SELECT public.current_user_rol()) = 'SuperAdministrador'
