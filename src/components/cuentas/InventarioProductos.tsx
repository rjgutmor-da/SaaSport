...
const Modal: React.FC<{ titulo: string; onCerrar: () => void; children: React.ReactNode; amplio?: boolean }> = ({ titulo, onCerrar, children, amplio = false }) => (
  <div className="cxc-modal-overlay" onClick={onCerrar}>
    <div className={`cxc-modal${amplio ? ' inventario-historial-modal' : ''}`} style={amplio ? undefined : { maxWidth: '620px' }} onClick={e => e.stopPropagation()}>
...
const InventarioProductos: React.FC = () => {
  ...
  const [productoHistorialId, setProductoHistorialId] = useState('');
  ...
  const cargarHistorial = async (reiniciar = false, paginaSolicitada?: number) => {
    ...
    if (productoHistorialId) query = query.eq('catalogo_item_id', productoHistorialId);
    const { data, error: err } = await query;
    if (err) { setError(err.message); return; }
    setMovimientos((data ?? []) as Movimiento[]); if (reiniciar) setPagina(0);
  };

  useEffect(() => { if (modal === 'historial') void cargarHistorial(true); }, [modal, mesHistorial, tipoHistorial, productoHistorialId, sucursalId]);
